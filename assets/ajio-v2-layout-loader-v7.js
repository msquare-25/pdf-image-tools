(async function(){
  ...
  const stampLabel=`function stampLabel(page,row,font){
    ...
    const bottomBase=height*.065;
    const minX=width*.04;
    const maxW=width*.50;
    const maxH=height*.285;
    ...
    for(let pass=0;pass<8&&!fit;pass++){
      const lines=makeLines(working);
      for(let size=7.2;size>=4.4;size-=.2){
        const lh=size+3.0;
  ...
  try{
    ...
    s.textContent=code+'\n//# sourceURL=/assets/ajio-v2-final-v7-runtime.js';
