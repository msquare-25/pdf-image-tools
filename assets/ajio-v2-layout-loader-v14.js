(async function(){
  'use strict';
  window.AJIO_V2_LAYOUT_VERSION='v14-customer-order-id-generic';
  const source='/assets/ajio-v2-final-v5.js?v=20260627-order-id-generic';
  ...
  try{
    ...
    code=code.replace(/if\(!\/\^FN\\d\{6,\}\$\/i\.test\(id\)\)continue;/,"if(!id||id==='-'||id==='NA'||id==='N/A')continue;");
    ...
    s.textContent=code+'\n//# sourceURL=/assets/ajio-v2-final-v14-customer-order-id-runtime.js';
    document.body.appendChild(s);
    const st=document.getElementById('status');
    if(st)st.textContent='V14 Customer Order ID support loaded. Upload Label, Invoice and Excel files.';
  }catch(err){
    console.error(err);
